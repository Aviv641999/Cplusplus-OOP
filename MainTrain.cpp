...
int main() //Train
{
	...
	if( account.GetBalance() != 10000 )
		cout << "Error in adding a new Person to an account(-4) (edited)." << endl;
	...
	if( bank.GetTotal() != 15000 )
		cout << "Error in adding an account to the bank(-4) C1" << bank.GetTotal() << endl;

	if( bank.GetNumberOfAccounts() != 1 )
		cout << "Error in updating the a count of account in the bank(-4) C2" << endl;

	bank.AddAccount( Account( Person( "Dor Levi", 32145672 ), 10 ) );
	if( bank.GetTotal() != 15010 )
		cout << "Error in adding a new account to the bank (-4) C3" << endl;

	if( bank.GetNumberOfAccounts() != 2 )
		cout << "Error in updating the a count of account in the bank(-4) C4" << endl;
	
	/*****************************D**************************************/
	currentA = bank.GetAccounts()[0]->GetBalance();
	currentB = bank.GetAccounts()[1]->GetBalance();
	
	cout << "INFO: currentA: " << currentA << endl;	// 15000
	cout << "INFO: currentB: " << currentB << endl;	// 10

	bank.AddAccount( lior, 200 ); // 15210
	cout << "INFO: bank.GetTotal(): " << bank.GetTotal() << endl;
